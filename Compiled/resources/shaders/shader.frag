...
float ShadowCalc(vec4 TempFragPosLightSpace, vec3 LightDirection) {
    ...
    float Bias = max(0.005 * (1.0 - dot(normalize(Normal), normalize(LightDirection))), 0.002f);

    //float Dist = length(TempFragPosLightSpace.xyz);
    //float Bias = max(0.0008f * (1.0f - dot(normalize(Normal), normalize(LightDirection))), 0.0001f);
